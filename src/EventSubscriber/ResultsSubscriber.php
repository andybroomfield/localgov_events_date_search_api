...
final class ResultsSubscriber implements EventSubscriberInterface {
  ...
  public function processResults(ProcessingResultsEvent $event) {
    ...
    uksort($result_items, function($left, $right) use ($nids) {
      list($datasource_type, $datasource_id) = Utility::splitCombinedId($left);
      list($left_nid, $lang) = Utility::splitPropertyPath($datasource_id);
      list($datasource_type, $datasource_id) = Utility::splitCombinedId($right);
      list($right_nid, $lang) = Utility::splitPropertyPath($datasource_id);
